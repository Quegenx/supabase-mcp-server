...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (request.params.name) {
      ...
      case "updateTable": {
        ...
        try {
          const alterStatements = operations.map((op) => {
            switch (op.operation) {
              case "ADD":
                return `ADD COLUMN ${op.columnName} ${op.dataType}${
                  op.constraints ? " " + op.constraints : ""
                }`;
              case "DROP":
                return `DROP COLUMN ${op.columnName}`;
              case "ALTER":
                return `ALTER COLUMN ${op.columnName} TYPE ${op.dataType}${
                  op.constraints ? " " + op.constraints : ""
                }`;
              case "RENAME":
      ...
      case "deleteColumn": {
        const { tableName, columnName } = request.params.arguments as {
          tableName: string;
          columnName: string;
        };
        await client.query("BEGIN");
      ...
      case "deleteIndex": {
        const { indexName } = request.params.arguments as { indexName: string };
        await client.query("BEGIN");
        try {
      ...
      case "addConstraint": {
        const { tableName, constraintName, definition } = request.params.arguments as {
          tableName: string;
      ...
      case "removeConstraint": {
        const { tableName, constraintName } = request.params.arguments as {
          tableName: string;
          constraintName: string;
        };
        await client.query("BEGIN");
      ...
      case "createView": {
        const { viewName, definition } = request.params.arguments as {
          viewName: string;
          definition: string;
        };
        await client.query("BEGIN");
      ...
      case "deleteView": {
        const { viewName } = request.params.arguments as { viewName: string };
        await client.query("BEGIN");
        try {
      ...
      case "createFunction": {
        const { functionDefinition } = request.params.arguments as {
      ...
      case "updateFunction": {
        const { functionName, functionDefinition } = request.params.arguments as {
          functionName: string;
          functionDefinition: string;
        };
        await client.query("BEGIN");
      ...
      case "updateSchema": {
        const { oldName, newName } = request.params.arguments as {
          oldName: string;
          newName: string;
        };
        await client.query("BEGIN");
      ...
      case "deleteSchema": {
        const { schemaName, cascade } = request.params.arguments as {
          schemaName: string;
          cascade?: boolean;
        };
        await client.query("BEGIN");
      ...
      case "importData": {
        const { tableName, records } = request.params.arguments as {
          tableName: string;
      ...
      case "updateUser": {
        const { username, newPassword } = request.params.arguments as {
          username: string;
          newPassword?: string;
        };
        if (!newPassword) {
